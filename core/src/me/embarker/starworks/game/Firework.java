...
public class Firework extends Group {
	...
	public static float FIREWORK_INTERVAL_IN_SEC = 1.0F; // How many sec will it take for another firework to spawn.
	...
	public Firework(int x) {
		...
		imgHead.setColor(Color.DARK_GRAY);
		imgHead.setPosition(
		...
		float bloomScale = (rand.nextInt(6) + 8) / 10.0F;
		Image imgBloom = new ImageMaker(Assets.FIREWORK_BLOOM,
				-8,
				10 - Assets.FIREWORK_BLOOM.getWidth() / 2).getImage();
